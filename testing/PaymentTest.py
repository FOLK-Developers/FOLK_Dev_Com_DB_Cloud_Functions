...
payment={'paid_chaintanya_coins':'0','paid_rupees':'0','paid_amount':'0','payment_method':'0','doc_id':'3'}
...
data2={'paid_chaintanya_coins':paid_chaintanya_coins,'paid_rupees':paid_rupees,'time':int(time.time()),'paid_amount':paid_amount,'doc_id':doc_id2}
ref2 = db.collection("ProfileTest").document(doc_id).collection("PaymentsFeature").document()
ref2.set(data2)

products={'cost':'0','discounts':'0','image_link':'0','name':'','type':'','size':'0','doc_id':'4'}
cost=products['cost']
...
data3={'cost':cost,'discount':discount,'image_link':image_link,'name':name,'type':type,"size":size,'doc_id':doc_id3}
ref3 = db.collection("ProfileTest").document(doc_id).collection("ProductsFeature").document()
ref3.set(data3)

referral={'amount':'0','referral_name':'','referral_profile_id':'','referral_email_id':'','doc_id':'5'}
